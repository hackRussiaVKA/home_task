...
if x == 'да':
    ...
    Devushki_dyn = []
    for s, (name_1, age_1) in enumerate(Students):
        for v, n in enumerate(name_1):
            if name_1[v] == ' ':
                Name_gender = name_1[0:v]
                if Name_gender == 'Илья' or Name_gender == 'Никита':
                    pass
                elif name_1[v - 1] in Gl_letters:
                    Devushki_dyn.append(name_1)
                else:
                    pass
            else:
                pass
    Devushki_dyn = sorted((Devushki_dyn))
    print(Devushki_dyn)
    f = open('1.txt', 'w')
    f.write(str(Devushki_dyn) + '\n')
    f.close()

else:
    print('Тогда используется следующий список :')
